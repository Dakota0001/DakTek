...
function EFFECT:Init( data )
	...
	for i = 1,5 do
		...
		if (particle) then
			...
			particle:SetColor(math.random(0,0),math.random(255,255),math.random(155,155),255)
	...
	if not(Ent==NULL) then
		for i = 1,5 do
			local particle = emitter:Add( "sprites/light_glow02_add.vmt", Pos +Ent:GetForward()*math.Rand(0,150)+ (Ent:GetUp()*12.5*math.sin(CurTime()*50)) + (Ent:GetRight()*12.5*math.cos(CurTime()*50))) 
	...
	if not(Ent==NULL) then
		...
		if not(Ent==NULL) then
		for i = 1,5 do
			local particle = emitter:Add( "sprites/light_glow02_add.vmt", Pos +Ent:GetForward()*math.Rand(0,150)+ (Ent:GetUp()*12.5*math.sin(CurTime()*25)) + (-Ent:GetRight()*12.5*math.cos(CurTime()*25))) 
