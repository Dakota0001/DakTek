...
function EFFECT:Init( data )
	...
	for i = 1,50 do
		...
		local particle = emitter:Add( "dak/smokey", Pos + Vector( math.random(-2.5,2.5),math.random(-2.5,2.5),math.random(-2.5,2.5) ) ) 
		...
		if (particle) then
			...
			particle:SetAirResistance(12.5)  
	...
	for i = 1,50 do
		...
		local particle = emitter:Add( "dak/flamelet5", Pos + Vector( math.random(-2.5,2.5),math.random(-2.5,2.5),math.random(-2.5,2.5) ) ) 
	...
	for i = 1,25 do
		...
		if (particle) then
			...
			particle:SetEndAlpha(0)
			particle:SetStartSize(50*math.Rand(0.9,1.1)) 
			particle:SetEndSize(0)
			particle:SetAngles( Angle(0,0,0) )
			particle:SetAngleVelocity( Angle(0,0,0) ) 
			particle:SetRoll(math.Rand( 0, 360 ))
