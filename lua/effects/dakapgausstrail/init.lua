...
function EFFECT:Init( data )
	...
	for i = 1,10 do
		...
		if (particle) then
			...
			particle:SetEndSize(12.5)
			particle:SetAngles( Angle(0,0,0) )
			particle:SetAngleVelocity( Angle(25,0,0) ) 
			particle:SetRoll(math.Rand( 0, 360 ))
			particle:SetColor(math.random(255,255),math.random(25,50),math.random(25,50),math.random(255,255))
	...
	if not(Ent==NULL) then
		for i = 1,5 do
			local particle = emitter:Add( "sprites/light_glow02_add.vmt", Pos +Ent:GetForward()*math.Rand(0,0)+ (Ent:GetUp()*5*math.sin(CurTime()*500)) + (Ent:GetRight()*5*math.cos(CurTime()*500))) 
			...
			if (particle) then
				...
				particle:SetRoll(math.Rand( 0, 360 ))
				particle:SetColor(math.random(255,255),math.random(0,50),math.random(0,0),255)
				particle:SetGravity( Vector(0,0,0) ) 
				particle:SetAirResistance(0.0)  
				particle:SetCollide(false)
				particle:SetBounce(0)
	...
	if not(Ent==NULL) then
		...
		if (particle) then
			...
			particle:SetStartAlpha(100)
			particle:SetEndAlpha(0)
			particle:SetStartSize(2.5+(size*2)) 
			particle:SetEndSize(0)
			particle:SetAngles( Angle(0,0,0) )
			particle:SetAngleVelocity( Angle(0,0,0) ) 
			particle:SetRoll(math.Rand( 0, 360 ))
			particle:SetColor(math.random(255,255),math.random(25,50),math.random(25,50),math.random(150,255))
