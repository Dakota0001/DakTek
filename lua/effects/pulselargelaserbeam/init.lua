	...
	if not(Ent==NULL) then
		for i = 1, 1 do
			...
			if (particle) then
				...
				particle:SetColor(0,255,255,math.random(200,255))
...
function EFFECT:Render()
	local Laser = Material( "dak/beamlargepulse" )
	render.SetMaterial( Laser )
	--render.SetColorModulation( 1, 0, 0 )
	if self.size>0.80 then
		render.DrawBeam( self.Startpos, self.Endpos, (1-self.size)*85.713, 0, 0.5, Color( 255, 200, 255, 255 ) ) 
	end
	if self.size<=0.80 and self.size>0.7 then
		render.DrawBeam( self.Startpos, self.Endpos, ((self.size))*21.384, 0, 0.5, Color( 255, 200, 255, 255 ) ) 
	end
	if self.size<=0.70 and self.size>=0.2 then
		render.DrawBeam( self.Startpos, self.Endpos, 15, 0, 0.5, Color( 255, 200, 255, 255 ) ) 
	end
	if self.size<0.2 then
		render.DrawBeam( self.Startpos, self.Endpos, self.size*50, 0, 0.5, Color( 255, 200, 255, 255 ) ) 
	end
end