...
function EFFECT:Init( data )
	...
	for i = 1,25 do
		...
		local particle = emitter:Add( "dak/smokey", Pos + Vector( math.random(-1.25,1.25),math.random(-1.25,1.25),math.random(-1.25,1.25) ) ) 
		...
		if (particle) then
			...
			particle:SetAirResistance(6.25)  
...
for i = 1,25 do
		...
		local particle = emitter:Add( "dak/flamelet5", Pos + Vector( math.random(-1.25,1.25),math.random(-1.25,1.25),math.random(-1.25,1.25) ) ) 
	...
	for i=1, 25 do
		...
		if (Debris) then
			Debris:SetVelocity (Vector(math.random(-15,15),math.random(-15,15),math.random(-15,15)))
