...
function EFFECT:Init( data )
	...
	for i = 1,3 do
		...
		local particle = emitter:Add( "dak/smokey", Pos + Ent:GetForward()*math.random( -337, 500 ) + Vector( math.random(-0.5,0.5),math.random(-0.5,0.5),math.random(-0.5,0.5) ) ) 
		 
		if particle == nil then particle = emitter:Add( "dak/smokey", Pos + Ent:GetForward()*math.random( -0, 0 ) + Vector(   math.random(-0.5,0.5),math.random(-0.5,0.5),math.random(-0.5,0.5) ) ) end
		...
		if (particle) then
			...
			particle:SetColor(CVal,CVal,CVal,math.random(50,100))
