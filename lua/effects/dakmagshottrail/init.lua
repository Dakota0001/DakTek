...
function EFFECT:Init( data )
	...
	for i = 1,10 do
		...
		if (particle) then
			...
			particle:SetColor(math.random(255,255),math.random(125,150),math.random(125,150),math.random(255,255))
	...
	if not(Ent==NULL) then
		for i = 1,5 do
			local particle = emitter:Add( "sprites/light_glow02_add.vmt", Pos +Ent:GetForward()*math.Rand(0,0)+ (Ent:GetUp()*6.25*math.sin(CurTime()*500)) + (Ent:GetRight()*6.25*math.cos(CurTime()*500))) 
	...
	if not(Ent==NULL) then
		...
		if (particle) then
			...
			particle:SetAngleVelocity( Angle(0,0,0) ) 
			particle:SetRoll(math.Rand( 0, 360 ))
			particle:SetColor(math.random(255,255),math.random(125,150),math.random(125,150),math.random(150,255))
