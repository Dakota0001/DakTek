...
if (CLIENT) then
	...
	function ENT:OnRemove()
		sound.Play( "dak/longtomimpact.wav", LocalPlayer():GetPos()+((self:GetPos()-LocalPlayer():GetPos()):GetNormalized()*1000), 100, 100, math.Clamp(2000/LocalPlayer():GetPos():Distance(self:GetPos()),0,1) )
	end
end
...
function ENT:Think()
	...
	if self.damage <= 0 or self.LastHitEnt:IsWorld() then
		...
		--sound.Play( "dak/longtomimpact.wav", Hit.HitPos, 100, 100, 1 )
		self:Remove()
	end	
