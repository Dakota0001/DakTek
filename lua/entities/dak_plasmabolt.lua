...
function ENT:Draw()
	...
	util.Effect("plasmatrail", effectdata)
...
if (CLIENT) then
	...
	function ENT:OnRemove()
		sound.Play( "dak/plasmacannonimpact.wav", LocalPlayer():GetPos()+((self:GetPos()-LocalPlayer():GetPos()):GetNormalized()*1000), 100, 75, math.Clamp(1500/LocalPlayer():GetPos():Distance(self:GetPos()),0,1) )
	end
end
...
function ENT:Think()
	...
	if self.damage <= 0 or self.LastHitEnt:IsWorld() then
		local effectdata = EffectData()
		...
		if table.Count(Targets) > 0 then
			for i = 1, #Targets do
				if Targets[i]:GetClass() == "dak_engine" then
					...
					Targets[i].DakHeat = Targets[i].DakHeat+5
				end
				if Targets[i]:IsPlayer() then
					if not Targets[i]:InVehicle() then
						...
						if Targets[i]:GetPos():Distance(self.LastHit) <= 250 then
							local ExpPain = DamageInfo()
							ExpPain:SetDamageForce( Vector(0,0,0) )
							ExpPain:SetDamage( 2500*(1-(Targets[i]:GetPos():Distance(self.LastHit)/1000)) )
							ExpPain:SetAttacker( self.DakGun.DakOwner )
							ExpPain:SetInflictor( self.DakGun )
							ExpPain:SetReportedPosition( self.LastHit )
							ExpPain:SetDamagePosition( Targets[i]:GetPos() )
							ExpPain:SetDamageType(DMG_DISSOLVE)
							Targets[i]:TakeDamageInfo( ExpPain )
						end
					end
				end
		...
		--sound.Play( "dak/plasmacannonimpact.wav", Hit.HitPos, 100, 75, 1 )
		self:Remove()
	end	
