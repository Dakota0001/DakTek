...
function ENT:Initialize()
	...
	self.Inputs = Wire_CreateInputs(self, { "Active","Gyro [ENTITY]" })
...
function ENT:Think()
    ...
    self.Gyro = self.Inputs.Gyro.Value

    if IsValid(self.DakEngine) then
    	if IsValid(self.Gyro) then
    		if self.Active>0 then
    			if CurTime()>=self.HeatTime+1 then
    				self.Heat = self.Heat + 0.5
    			end
    			self.Gyro.DakMASCon = 1
    		else
    			if CurTime()>=self.HeatTime+1 then
    				if self.Heat>0 then
    					self.Heat = self.Heat - 0.5
    			...
    			self.Gyro.DakMASCon = 0
    		end
    		if CurTime()>=self.HeatTime+1 then
	    		if self.Heat>0 then
	    			local EngineHeat = self.DakEngine.DakHeat
					local EngineHeat = EngineHeat + self.Heat
					self.DakEngine.DakHeat = EngineHeat
	    		end
	    		self.HeatTime = CurTime()
	    	end
    	end
    end
