...
function ENT:Draw()
	...
	util.Effect("erflamertrail", effectdata)
...
function ENT:Initialize()
	...
	self.DakHealth=500
 	...
 	self.Activated = 0
 	if IsValid(self.DakGun.DakEngine) then
...
function ENT:Think()
	...
	if self.LastHitEnt:IsValid() then
		...
		effectdata:SetMagnitude(.5)
		effectdata:SetScale(200)
		util.Effect("dakerflamerexplosion", effectdata)
		local Targets = ents.FindInSphere( self.LastHit, 100 )
