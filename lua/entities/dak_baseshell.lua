...
if (CLIENT) then
	...
	function ENT:OnRemove()
		sound.Play( self:GetNWString("ImpactSound"), LocalPlayer():GetPos()+((self:GetPos()-LocalPlayer():GetPos()):GetNormalized()*1000), 100, self:GetNWInt("ImpactPitch"), math.Clamp(1500/LocalPlayer():GetPos():Distance(self:GetPos()),0,1) )
	end
end
			...
			if table.Count(Targets) > 0 then
				for i = 1, #Targets do
					if Targets[i]:GetClass() == "dak_engine" then
						...
						Targets[i]:Ignite(self.DakBurnTime,1)
						Targets[i].DakHeat = Targets[i].DakHeat+self.DakBurnHeat
					end
