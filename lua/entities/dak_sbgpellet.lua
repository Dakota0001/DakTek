...
function ENT:Draw()
	...
	util.Effect("daksbgausstrail", effectdata)
...
if (CLIENT) then
	...
	--	sound.Play( "dak/gaussimpact.wav", LocalPlayer():GetPos()+((self:GetPos()-LocalPlayer():GetPos()):GetNormalized()*1000), 80, 120, math.Clamp(1500/LocalPlayer():GetPos():Distance(self:GetPos()),0,1) )
	--end
end
...
function ENT:Think()
	...
	if self.damage <= 0 or self.LastHitEnt:IsWorld() then
		...
		effectdata:SetMagnitude(.5)
		effectdata:SetScale(200)
		util.Effect("daksbgaussexplosion", effectdata)
			if(self.LastHitEnt:IsValid()) then
		...
		sound.Play( "dak/gaussimpact.wav", Hit.HitPos, 80, 120, 1 )
		self:Remove()
	end
